...
async def book_detail(book_no: int, request: Request, db: Session=Depends(get_db)):
    ...
    if form.is_vaild():
        try:
            ...
            grade = CreateGrade(user_no=current_user.user_no, book_no=book_no, value=form.__dict__.get("value"))
            grade = create_new_grade(grade=grade, db=db)
            if grade:
                return responses.RedirectResponse(f"/detail/{book_no}?msg=Succesfully-Save-Rating", status_code=status.HTTP_302_FOUND)
            else:
                return responses.RedirectResponse(f"/detail/{book_no}?msg=Succesfully-Update-Rating", status_code=status.HTTP_302_FOUND)
